    ...
    echo '<br>';
      ...
      else if(ctype_lower($Password ))                                                    
      {
        ...
        (($ConfirmPassword != $Password))
         ...
         {
          $confirmPasswordErr = 'password is matched';
         }
        ...
        if(empty($Date))
{
  ...
  /* if(!empty($FirsName)&&!empty($LastName)&&!empty($Username)&&!empty($Password))
...
{
    echo 'Registration has Successfully completed';
}
else
echo 'No Data Saved';
}
...
$mydb = new db();
        ...
        $result = $mydb->insertUser($FirsName, $LastName,$Password,$ConfirmPassword, $Email,$Username,$Phone,$Street,$City,$Postal,$gender,$Date, $conn);
        ...
        {
            echo "Can't insert data" . $conn->error;
        }
}
...
{
    header("location: ../view/touristlogin.php");
}
